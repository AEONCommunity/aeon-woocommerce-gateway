<?php
/*
 * Main Gateway of Aeon using either a local daemon or the explorer
 * Authors: Serhack, cryptochangements, mosu-forge
 * Ported to aeon: 420coupe
 */

defined( 'ABSPATH' ) || exit;

require_once('class-aeon-cryptonote.php');

class Aeon_Gateway extends WC_Payment_Gateway
{
    private static $_id = 'aeon_gateway';
    ...
    private static $use_aeon_price = false;                                
    private static $use_aeon_price_decimals = AEON_GATEWAY_ATOMIC_UNITS; 

    private static $cryptonote;
    private static $aeon_wallet_rpc;
    private static $aeon_explorer_tools;
    private static $log;
    ...
    {
        return apply_filters('woocommerce_gateway_icon', '<img src="'.AEON_GATEWAY_PLUGIN_URL.'assets/images/aeon-icon.png"/>', $this->id);
    }

    function __construct($add_action=true)
    {
        $this->id = self::$_id;
        $this->method_title = __(self::$_method_title, 'aeon_gateway');
        $this->method_description = __(self::$_method_description, 'aeon_gateway');
        ...
        self::$address = $this->settings['aeon_address'];
        ...
        self::$use_aeon_price = $this->settings['use_aeon_price'] == 'yes';
        self::$use_aeon_price_decimals = $this->settings['use_aeon_price_decimals'];

        $explorer_url = self::$testnet ? AEON_GATEWAY_TESTNET_EXPLORER_URL : AEON_GATEWAY_MAINNET_EXPLORER_URL;
        defined('AEON_GATEWAY_EXPLORER_URL') || define('AEON_GATEWAY_EXPLORER_URL', $explorer_url);

        if($add_action)
            add_action('woocommerce_update_options_payment_gateways_'.$this->id, array($this, 'process_admin_options'));

        // Initialize helper classes
        self::$cryptonote = new Aeon_Cryptonote();
        if(self::$confirm_type == 'aeon-wallet-rpc') {
            require_once('class-aeon-wallet-rpc.php');
            self::$aeon_wallet_rpc = new Aeon_Wallet_Rpc(self::$host, self::$port);
        } else {
            require_once('class-aeon-explorer-tools.php');
            self::$aeon_explorer_tools = new Aeon_Explorer_Tools(self::$testnet);
        }
    ...
    {
        $this->form_fields = include 'admin/aeon-gateway-admin-settings.php';
    }
    ...
    public function admin_options()
    {
        ...
        if($confirm_type === 'aeon-wallet-rpc')
        ...
        include AEON_GATEWAY_PLUGIN_DIR . '/templates/aeon-gateway/admin/settings-page.php';
    }
    ...
    public static function admin_balance_info()
    {
        ...
        $wallet_amount = self::$aeon_wallet_rpc->getbalance();
        $height = self::$aeon_wallet_rpc->getheight();
        if (!isset($wallet_amount)) {
            self::$_errors[] = 'Cannot connect to aeon-wallet-rpc';
            self::$log->add('Aeon_Payments', '[ERROR] Cannot connect to aeon-wallet-rpc');
            return array(
                'height' => 'Not Available',
                'balance' => 'Not Available',
                'unlocked_balance' => 'Not Available',
            ...
            return array(
                'height' => $height,
                'balance' => self::format_aeon($wallet_amount['balance']).' Aeon',
                'unlocked_balance' => self::format_aeon($wallet_amount['unlocked_balance']).' Aeon'
            );
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        if(self::$confirm_type != 'aeon-wallet-rpc') {
          ...
          // Generate subaddress
          $payment_id = self::$aeon_wallet_rpc->create_address(0, 'Order: ' . $order_id);
        ...
        $aeon_amount = 1e8 * $fiat_amount / $rate;

        if(self::$discount)
            $aeon_amount = $aeon_amount - $aeon_amount * self::$discount / 100;

        $aeon_amount = intval($aeon_amount * AEON_GATEWAY_ATOMIC_UNITS_POW);

        $query = $wpdb->prepare("INSERT INTO $table_name (order_id, payment_id, currency, rate, amount) VALUES (%d, %s, %s, %d, %d)", array($order_id, $payment_id, $currency, $rate, $aeon_amount));
        $wpdb->query($query);

        $order->update_status('on-hold', __('Awaiting offline payment', 'aeon_gateway'));
    ...
    public static function do_update_event()
    {
        ...
        $api_link = 'https://min-api.cryptocompare.com/data/price?fsym=AEON&tsyms='.$currencies.'&extraParams=aeon_woocommerce';
        $curl = curl_init();
        ...
        if(self::$confirm_type == 'aeon-wallet-rpc')
            ...
            $height = self::$aeon_explorer_tools->getheight();
        set_transient('aeon_gateway_network_height', $height);
        ...
        foreach($pending_payments as $pending) {
            ...
            $amount_aeon = $quote->amount_total;
            ...
            $paid = $amount_paid > $amount_aeon - AEON_GATEWAY_ATOMIC_UNIT_THRESHOLD;
            ...
            if($paid && $confirmed) {
                self::$log->add('Aeon_Payments', "[SUCCESS] Payment has been confirmed for order id $order_id and payment id $payment_id");
                ...
                if(self::is_virtual_in_cart($order_id) == true){
                    $order->update_status('completed', __('Payment has been received.', 'aeon_gateway'));
                } else {
                    $order->update_status('processing', __('Payment has been received.', 'aeon_gateway'));
                }

            } else if($paid) {
                self::$log->add('Aeon_Payments', "[SUCCESS] Payment has been received for order id $order_id and payment id $payment_id");
                ...
                if($order_age_seconds > self::$valid_time) {
                    self::$log->add('Aeon_Payments', "[FAILED] Payment has expired for order id $order_id and payment id $payment_id");
                    $query = $wpdb->prepare("UPDATE $table_name_1 SET pending=0 WHERE payment_id=%s", array($payment_id));
                    $wpdb->query($query);

                    unset(self::$payment_details[$order_id]);

                    $order->update_status('cancelled', __('Payment has expired.', 'aeon_gateway'));
                }
            }
        }
    }
    ...
    {
        $txs = array();
        $address_index = self::$aeon_wallet_rpc->get_address_index($subaddress);
        if(isset($address_index['index']['minor'])){
          $address_index = $address_index['index']['minor'];
        }
        else {
          self::$log->add('aeon_gateway', '[ERROR] Couldn\'t get address index of subaddress: ' . $subaddress);
          return $txs;
        }
        $payments = self::$aeon_wallet_rpc->get_transfers(array( 'in' => true, 'pool' => true, 'subaddr_indices' => array($address_index)));
    ...
    {
        $txs = array();
        $outputs = self::$aeon_explorer_tools->get_outputs(self::$address, self::$viewkey);
        ...
        if (count($details)) {
            ...
            foreach($details as $tx) {
                ...
                $txs[] = array(
                    ...
                    'amount_formatted' => self::format_aeon($tx->amount_paid)
                );
            ...
            if(count($heights) && !in_array(0, $heights)) {
                $height = get_transient('aeon_gateway_network_height');
                $highest_block = max($heights);
            ...
            $time_to_confirm = self::format_seconds_to_time($blocks_to_confirm * AEON_GATEWAY_DIFFICULTY_TARGET);
            ...
            if(self::$confirm_type == 'aeon-wallet-rpc') {
                ...
                if ($address) {
                    ...
                    self::$log->add('aeon_gateway', '[ERROR] Merchant has not set Aeon address');
                    return '[ERROR] Merchant has not set Aeon address';
                }
            }
            ...
            $amount_formatted = self::format_aeon($amount_due);
            $qrcode_uri = 'aeon:'.$address.'?tx_amount='.$amount_formatted.'&tx_payment_id='.$payment_id;
            ...
            $payment_details = array(
                ...
                'amount_total_formatted' => self::format_aeon($amount_total),
                'amount_paid_formatted' => self::format_aeon($amount_paid),
                'amount_due_formatted' => self::format_aeon($amount_due),
                'status' => $status,
    ...
    public static function get_payment_details_ajax() {
        ...
        if($order->get_payment_method() != self::$_id)
            self::ajax_output(array('error' => '[ERROR] Order not paid for with Aeon'));
    ...
    public static function admin_order_page($post)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include AEON_GATEWAY_PLUGIN_DIR . '/templates/aeon-gateway/admin/order-history-error-page.php';
            return;
        }
        include AEON_GATEWAY_PLUGIN_DIR . '/templates/aeon-gateway/admin/order-history-page.php';
    }
    ...
    public static function customer_order_page($order)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include AEON_GATEWAY_PLUGIN_DIR . '/templates/aeon-gateway/customer/order-error-page.php';
            return;
        }
        $show_qr = self::$show_qr;
        $details_json = json_encode($details);
        $ajax_url = WC_AJAX::get_endpoint('aeon_gateway_payment_details');
        include AEON_GATEWAY_PLUGIN_DIR . '/templates/aeon-gateway/customer/order-page.php';
    }
    ...
    public static function customer_order_email($order)
    {
        ...
        $details = self::get_payment_details($order_id);
        if(!is_array($details)) {
            include AEON_GATEWAY_PLUGIN_DIR . '/templates/aeon-gateway/customer/order-email-error-block.php';
            return;
        }
        include AEON_GATEWAY_PLUGIN_DIR . '/templates/aeon-gateway/customer/order-email-block.php';
    }
    ...
    public static function use_aeon_price()
    {
        return self::$use_aeon_price;
    }
    ...
    {
        $rate = self::get_live_rate($currency);
        $aeon_amount = intval(AEON_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / AEON_GATEWAY_ATOMIC_UNITS_POW;
        $aeon_amount_formatted = sprintf('%.'.self::$use_aeon_price_decimals.'f', $aeon_amount);

        return <<<HTML
            <span class="woocommerce-Price-amount amount" data-price="$price" data-currency="$currency"
        data-rate="$rate" data-rate-type="live">
            $aeon_amount_formatted
            <span class="woocommerce-Price-currencySymbol">AEON</span>
        </span>
...
HTML;
    ...
    public static function convert_wc_price_order($price_html, $order)
    {
        ...
        $aeon_amount = intval(AEON_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / AEON_GATEWAY_ATOMIC_UNITS_POW;
        $aeon_amount_formatted = sprintf('%.'.AEON_GATEWAY_ATOMIC_UNITS.'f', $aeon_amount);
...
HTML;
    ...
    public static function format_aeon($atomic_units) {
        return sprintf(AEON_GATEWAY_ATOMIC_UNITS_SPRINTF, $atomic_units / AEON_GATEWAY_ATOMIC_UNITS_POW);
    }
