...
class Aeon_Wallet_Rpc
{
    ...
    protected function & getResponse(&$pRequest)
    {
        ...
        if (0 < curl_errno($ch)) {
            if(is_admin())
                echo '[ERROR] Failed to connect to aeon-wallet-rpc at ' . $this->host . ' port '. $this->port .'</br>';
        }
    ...
    /*
     * The following functions can all be called to interact with the Aeon RPC wallet
    ...
    public function make_uri($address, $amount, $recipient_name = null, $description = null)
    {
        ...
        $new_amount = $amount * AEON_GATEWAY_ATOMIC_UNITS_POW;
